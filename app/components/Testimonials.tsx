...
function TestimonialCard({ name, review, image }: Testimonial) {
  return (
    <figure className="p-6 rounded-2xl bg-indigo-50 ring-1 ring-indigo-200/60 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition">
      <div className="flex items-center gap-4 mb-3">
        <Avatar name={name} src={image} />
        <figcaption className="font-semibold text-slate-900">{name}</figcaption>
      </div>
      ...
      <div className="relative pl-6">
        {/* quote mark */}
        <svg
          className="absolute left-0 top-1 w-4 h-4 text-indigo-400"
          viewBox="0 0 24 24"
          fill="currentColor"
          aria-hidden="true"
        >
